...
export default function AdminProducts() {
  ...
  const [form, setForm] = useState({ name: '', price: '', categoryId: '', description: '', imageUrl: '', discountedPrice: '', preparationTime: '15', isFeatured: false, ingredients: '' });

  const fetchData = async () => {
    const [p, c] = await Promise.all([api.getAdminProducts(), api.getAdminCategories()]);
  ...
  const openNew = () => { setEditing(null); setForm({ name: '', price: '', categoryId: categories[0]?.id || '', description: '', imageUrl: '', discountedPrice: '', preparationTime: '15', isFeatured: false, ingredients: '' }); setShowModal(true); };
  const openEdit = (p: any) => { setEditing(p); setForm({ name: p.name, price: String(p.price), categoryId: p.categoryId, description: p.description || '', imageUrl: p.imageUrl || '', discountedPrice: p.discountedPrice ? String(p.discountedPrice) : '', preparationTime: String(p.preparationTime || 15), isFeatured: p.isFeatured, ingredients: p.ingredients || '' }); setShowModal(true); };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr><th className="text-left p-3">Nome</th><th className="text-left p-3">Categoria</th><th className="text-left p-3">Preço</th><th className="text-left p-3">Destaque</th><th className="text-left p-3">Ativo</th><th className="text-left p-3">Ações</th></tr>
            </thead>
            <tbody className="divide-y">
              {products.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">{p.isFeatured ? 'Sim' : 'Não'}</td>
